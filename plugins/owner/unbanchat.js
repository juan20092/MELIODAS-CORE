import { removeBannedChat } from '../../wzr/banlist.js'
  ...
  try {
    ...
    if (!m.isGroup) {
      return conn.sendMessage(
        m.chat,
        {
          text: '𝙴𝚂𝚃𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝚂𝙾𝙻𝙾 𝙵𝚄𝙽𝙲𝙸𝙾𝙽𝙰 𝙴𝙽 𝙶𝚁𝚄𝙿𝙾𝚂',
          contextInfo: {
            forwardingScore: 1,
            isForwarded: true,
    ...
    const wasRemoved = removeBannedChat(m.chat)

    await conn.sendMessage(
      m.chat,
      {
        text: wasRemoved
          ? '🔰 𝙴𝙻 𝙱𝙾𝚃 𝙵𝚄𝙴 𝚁𝙴𝙰𝙲𝚃𝙸𝚅𝙰𝙳𝙾 𝙴𝙽 𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾'
          : '❇️ 𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾 𝙽𝙾 𝙴𝚂𝚃𝙰𝙱𝙰 𝙳𝙴𝚂𝙰𝙲𝚃𝙸𝚅𝙰𝙳𝙾',
        contextInfo: {
          forwardingScore: 1,
          isForwarded: true,
          forwardedNewsletterMessageInfo: {
    ...
    await conn.sendMessage(
      m.chat,
      {
        text: '🚫 𝙽𝙾 𝚂𝙴 𝙿𝚄𝙳𝙾 𝚁𝙴𝙰𝙲𝚃𝙸𝚅𝙰𝚁 𝙴𝙻 𝙱𝙾𝚃 𝙴𝙽 𝙴𝚂𝚃𝙴 𝙶𝚁𝚄𝙿𝙾',
        contextInfo: {
          forwardingScore: 1,
          isForwarded: true,
          forwardedNewsletterMessageInfo: {
